...
const generateCompletionCertificate = async (req, res) => {
  try {
    ...
    // 🎯 CRITICAL: Use student-provided certificate details from ENROLLMENT FORM ONLY
    if (!progress.certificateDetails || !progress.certificateDetails.fullName) {
      console.warn("⚠️ Certificate details missing in CourseProgress, falling back for legacy support but this shouldn't happen for new enrollments.");
    }
    
    const studentNameToPrint = progress.certificateDetails?.fullName || user.userName || user.userEmail || userId;
    const studentEmailToPrint = progress.certificateDetails?.email || user.userEmail;
    const studentOrgToPrint = progress.certificateDetails?.organization || "";
    
    // Guardian name - Enrollment form is the ONLY source for new enrollments
    const fatherNameToPrint = progress.certificateDetails?.guardianName || user.guardianDetails || "";
