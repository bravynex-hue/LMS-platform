...
const createAdvancedRateLimit = (options) => {
  ...
  if (process.env.NODE_ENV !== "production") {
    return (req, res, next) => next(); // Completely bypass in development
  }
  ...
  const limiter = rateLimit({
    ...
    store: process.env.NODE_ENV === "production"
      ? new RedisStore({
          ...
          prefix: `rl:sec:${crypto.randomBytes(4).toString('hex')}:`,
        })
      : undefined,
    handler: (req, res) => {
...
const bruteForceProtection = createAdvancedRateLimit({
  ...
  max: process.env.NODE_ENV === "production" ? 10 : 300, // 10 attempts per 15 minutes in PROD, 300 in DEV
...
const registrationProtection = createAdvancedRateLimit({
  ...
  max: process.env.NODE_ENV === "production" ? 3 : 100, // 3 in PROD, 100 in DEV
