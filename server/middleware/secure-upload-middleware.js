...
const validateUploadedFiles = async (req, res, next) => {
  try {
    ...
    for (const file of files) {
      ...
      // Check for embedded scripts in file content, but ONLY for text-based files
      // Binary files (PDF, images, videos) can trigger false positives from random bytes
      const isTextBased = file.mimetype && (
      ...
      if (isTextBased) {
        ...
        const content = contentSource && Buffer.isBuffer(contentSource)
          ? contentSource.toString('utf8', 0, Math.min(4096, contentSource.length))
          : "";
