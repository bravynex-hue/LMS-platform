...
import { CheckCircle, Lock, PlayCircle, BookOpen, Loader2, Zap, Layers, ChevronRight, User } from "lucide-react";
...
function StudentViewCourseDetailsPage() {
  ...
  const [enrollmentFormData, setEnrollmentFormData] = useState(initialEnrollmentFormData);
  ...
  async function handleCreatePayment() {
    if (!auth?.authenticate) return navigate('/signin');
    if (isPurchased) return navigate(`/learn/${studentViewCourseDetails?._id}`);
    
    // First, show enrollment modal to collect certificate details
    ...
    setEnrollmentFormData({
      ...
      certificateGuardianName: auth?.user?.guardianDetails || "",
  ...
  function isEnrollmentFormValid() {
    ...
    const guardianName = enrollmentFormData?.certificateGuardianName?.trim() || "";
    const email = enrollmentFormData?.certificateEmail?.trim() || "";

    return (
      fullName.length <= 12 &&
      guardianName.length <= 12 &&
      email !== ""
    );
  }
  ...
  async function handleEnrollmentSubmit(event) {
    ...
    if (!isEnrollmentFormValid()) {
      alert("Please provide valid Full Name, Guardian Name (min 12 characters each), and Email to proceed.");
      return;
    }
    ...
    try {
      const paymentPayload = {
        ...
        // Added certificate details from form
        ...enrollmentFormData,
      };
      ...
      // Hide modal once we have the payment data and are ready to open Razorpay
      setShowEnrollmentModal(false);
      // Handle test mode response from server
  ...
  return (
    <div className="min-h-screen text-gray-200" style={{ background: "var(--bg-dark)" }}>
              ...
              <div className="space-y-3">
                ...
                <div className="flex flex-wrap items-center gap-4 mt-4">
                  <div className="flex items-center gap-2">
                    ...
                    <span className="text-sm font-bold text-gray-400">By {studentViewCourseDetails?.instructorName}</span>
                  </div>
                  <div className="flex items-center gap-2">
      ...
      {/* Enrollment Certificate Details Modal */}
      <Dialog
        open={showEnrollmentModal}
        onOpenChange={() => {
          if (!isEnrolling) setShowEnrollmentModal(false);
        }}
      >
        <DialogContent className="bg-[#0a1428] border-white/10 text-gray-200 w-full max-w-xl p-0 overflow-hidden shadow-3xl">
          <div className="bg-white/[0.02] p-8 border-b border-white/5">
            ...
            <p className="text-gray-500 text-xs font-medium mt-2">
              Please provide your details correctly for certificate generation.
            </p>
            <div className="mt-4 space-y-1">
              <p className={`text-[10px] ${enrollmentFormData?.certificateFullName?.trim()?.length >= 12 ? "text-green-500" : "text-gray-400"}`}>
                • Full Name: Minimum 12 characters
              </p>
              <p className={`text-[10px] ${enrollmentFormData?.certificateGuardianName?.trim()?.length >= 12 ? "text-green-500" : "text-gray-400"}`}>
                • Guardian Name: Minimum 12 characters
              </p>
            </div>
          </div>
          ...
          <div className="p-6">
            <CommonForm
              formControls={enrollmentFormControls}
              formData={enrollmentFormData}
              setFormData={setEnrollmentFormData}
              buttonText={isEnrolling ? "Processing..." : "Confirm & Enroll"}
              handleSubmit={handleEnrollmentSubmit}
              isButtonDisabled={isEnrolling || !isEnrollmentFormValid()}
            />
          </div>
        </DialogContent>
      </Dialog>
