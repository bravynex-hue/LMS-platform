...
function RealTimeRevenueAnalysis({ listOfCourses = [] }) {
  ...
  useEffect(() => {
    ...
    async function load() {
      ...
      if (auth?.user?.role === "admin") {
        // Admin: compute stats from courses directly, analytics will be built in useMemo
        ...
        let todayRevenue = 0, todayStudents = 0;
        safe.forEach(c => {
          (c.students || []).forEach(st => {
            const eDate = st.enrollmentDate ? new Date(st.enrollmentDate) : null;
            if (eDate && eDate >= todayStart) {
              todayRevenue += Number(c.pricing || 0);
              todayStudents += 1;
            }
          });
        });
        setLiveStats({ totalRevenue, totalStudents, todayRevenue, todayStudents, lastEnrollment: null, lastUpdated: new Date().toLocaleTimeString() });
        return;
      }
  ...
  const revenueData = useMemo(() => {
    ...
    // Build enrollment events from all students across all courses
    const enrollmentEvents = [];
    safe.forEach(c => {
      const price = Number(c.pricing || 0);
      (c.students || []).forEach(st => {
        const eDate = st.enrollmentDate ? new Date(st.enrollmentDate) : (c.createdAt ? new Date(c.createdAt) : new Date());
        enrollmentEvents.push({ date: eDate, revenue: price });
      });
    });

    // Daily data: last 30 days - distribute with enrollment dates
    const dailyData = [];
    const dayKeyIndex = new Map();
    for (let i = 29; i >= 0; i--) {
      const d = new Date(now.getFullYear(), now.getMonth(), now.getDate() - i);
      const key = d.toISOString().slice(0, 10);
      dayKeyIndex.set(key, dailyData.length);
      dailyData.push({ day: d.toLocaleDateString("en-US",{month:"short",day:"numeric"}), revenue: 0, students: 0 });
    }
    enrollmentEvents.forEach(ev => {
      const key = ev.date.toISOString().slice(0, 10);
      if (dayKeyIndex.has(key)) {
        const idx = dayKeyIndex.get(key);
        dailyData[idx].revenue += ev.revenue;
        dailyData[idx].students += 1;
      }
    });
    ...
    for (let i = 11; i >= 0; i--) {
      ...
      monthKeyIndex.set(key, monthlyData.length);
      monthlyData.push({ month: d.toLocaleString("en-US",{month:"short"}), revenue: 0, students: 0 });
    }
    enrollmentEvents.forEach(ev => {
      ...
      if (monthKeyIndex.has(key)) {
        ...
        monthlyData[idx].revenue += ev.revenue;
    ...
    for (let i = 23; i >= 0; i--) {
      ...
      hourKeyIndex.set(key, hourlyData.length);
      hourlyData.push({ hour: `${String(d.getHours()).padStart(2,"0")}:00`, revenue: 0, students: 0 });
    }
    enrollmentEvents.forEach(ev => {
      const key = `${ev.date.getFullYear()}-${String(ev.date.getMonth()+1).padStart(2,'0')}-${String(ev.date.getDate()).padStart(2,'0')}T${String(ev.date.getHours()).padStart(2,'0')}`;
      if (hourKeyIndex.has(key)) {
        const idx = hourKeyIndex.get(key);
        hourlyData[idx].revenue += ev.revenue;
        hourlyData[idx].students += 1;
      }
    });
  ...
  const filteredTotalRevenue = chartData.reduce((acc, curr) => acc + (curr.revenue || 0), 0);
  const filteredTotalStudents = chartData.reduce((acc, curr) => acc + (curr.students || 0), 0);
  const filteredAvgRevenue = filteredTotalStudents > 0 ? filteredTotalRevenue / filteredTotalStudents : 0;

  const kpiCards = [
    { title: `${selectedPeriod.charAt(0).toUpperCase() + selectedPeriod.slice(1)} Revenue`, value: formatINR(filteredTotalRevenue), change: 12.5, icon: IndianRupee, accent: "emerald", isLive: true, liveValue: `+${formatINR(liveStats.todayRevenue)} today` },
    { title: `${selectedPeriod.charAt(0).toUpperCase() + selectedPeriod.slice(1)} Students`, value: filteredTotalStudents.toLocaleString(), change: 8.3, icon: Users, accent: "blue", isLive: true, liveValue: `+${liveStats.todayStudents} today` },
    { title: "Avg Revenue/Student", value: formatINR(filteredAvgRevenue > 0 ? filteredAvgRevenue : revenueData.averageRevenuePerStudent), change: 5.2, icon: Target, accent: "purple" },
